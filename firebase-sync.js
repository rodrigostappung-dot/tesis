...
window.FB = {
  ...
  async loadAll() {
    ...
    snap.forEach(doc => {
      ...
      results[mix] = {
        retraction: d.retraction || window.SHRINKAGE_SPECIMENS.map(s => ({ id: s.id, values: { 0: [], 1: [], 7: [], 28: [] } })),
        flexion: d.flexion || window.FLEXION_SPECIMENS.map(s => ({ id: s.id, age: s.age, file: null, parsed: null, length: '', height: '', width: '', weight: '', testDate: '', firstPeakIdx: null, trimIdx: null })),
        compression: d.compression || window.COMPRESSION_SPECIMENS.map(s => ({ id: s.id, age: s.age, file: null, parsed: null, length: '', height: '', width: '', weight: '', testDate: '', firstPeakIdx: null, trimIdx: null })),
      };
    });
