...
function ShrinkageViewer({ specimens, mix, T }) {
  // Una serie por probeta + una serie del promedio
  const series = [];
  const ages = window.SHRINKAGE_AGES;

  const PALETTE_RET = ['#1f77b4', '#ff7f0e'];

  specimens.forEach((spec, i) => {
    const s = window.shrinkageSeries(spec, ages);
    if (!s) return;
    series.push({
      points: s.points.map(p => ({ disp: p.t, load: p.change, t: p.t, stress: p.change, strain: 0, elong: 0 })),
  ...
  if (avgPts.length > 0) {
    series.push({
      points: avgPts.map(p => ({ disp: p.t, load: p.change, t: p.t, stress: p.change, strain: 0, elong: 0 })),
      color: '#2ca02c',
          ...
          <tbody>
              ...
              return (
                <tr key={spec.id}>
                  ...
                  {ages.map(a => {
                    ...
                    return <td key={a}>{pt ? pt.change.toFixed(0) : '—'}</td>;
                  })}
                </tr>
              );
            })}
            <tr className="avg-row">
              ...
              {ages.map(a => {
                ...
                return <td key={a} className={pt ? 'has-val' : 'empty'}>{pt ? pt.change.toFixed(0) : '—'}</td>;
              })}
            </tr>
          </tbody>
        </table>
...
function ComparatorView({ state, T, lang }) {
  ...
  const hasMech = selected.some(s => s.kind === 'mech');
  ...
  const series = selected.filter(s => visibleMap[s.key] !== false).map((s, i) => {
    ...
    if (s.kind === 'shrink') {
      ...
      return {
        points: seriesData.points.map(p => ({ disp: p.t, load: p.change, t: p.t, stress: p.change, strain: 0, elong: 0 })),
        color,
    ...
    if (s.kind === 'shrinkMix') {
      ...
      return {
        points: pts.map(p => ({ disp: p.t, load: p.change, t: p.t, stress: p.change, strain: 0, elong: 0 })),
        color,
      ...
      <div className="comp-layout">
        <div className="compare-plot-wrap">
          <CurvePlot
            ...
            yLabel={hasShrink && !hasMech ? (T.shrinkChange || 'Cambio Δ (μm/m)')
...
function CurvePicker({ state, onSelectMany, onClose, T }) {
  // Build items: mech (flexion / compression) + shrinkage per specimen + shrinkage mix-avg
  const items = [];
  ...
  const keyOf = (it) => `${it.mix}_${it.testKey}_${it.specimenId}_${it.kind}`;
  const toggle = (it) => {
    const k = keyOf(it);
    setPicked(p => {
      const n = new Set(p);
