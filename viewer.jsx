...
window.applyCorrection = function(parsed, corr) {
  ...
  const tail = pts.slice(i1).map(p => ({
    t: p.t, load: p.load, disp: p.disp - x0, stress: p.stress, strain: p.strain, elong: p.elong,
  }));
  ...
  return { ...parsed, points: corrected, pmax, smax, idxPmax, nPoints: corrected.length };
};
