...
// Corrección automática (solo flexión): punto 1 = carga más cercana a 0.4 kN,
// punto 2 = carga más cercana a 0.8 kN, tomando siempre la PRIMERA vez que la curva
// alcanza esa zona (por si hay más de un cruce). Solo aplica si la probeta llegó a 1 kN.
window.findAutoCorrectionPoints = function(parsed, targets) {
  targets = targets || [0.4, 0.8];
  if (!parsed || !parsed.points || !parsed.points.length) return null;
  const pts = parsed.points;
  let pmax = 0; pts.forEach(p => { if (p.load > pmax) pmax = p.load; });
  if (pmax < 1) return null; // no llegó a 1 kN: no se corrige automáticamente

  const findNearestAtFirstCrossing = (target, fromIdx) => {
    for (let i = Math.max(1, fromIdx); i < pts.length; i++) {
      if (pts[i].load >= target) {
        const dCur = Math.abs(pts[i].load - target);
        const dPrev = Math.abs(pts[i - 1].load - target);
        return dPrev <= dCur ? i - 1 : i;
      }
    }
    return null;
  };
  const i1 = findNearestAtFirstCrossing(targets[0], 0);
...
function IndividualViewer({ specimen, testKey, onUpdate, onAddToCompare, T }) {
  ...
  const allowCorrection = true; // corrección manual disponible en compresión y flexión
  const allowAutoCorrection = testKey === 'flexion'; // automática solo para flexión

  // Apply correction (corrige curva), then trim.
  const corrected = specimen.correction
  ...
  const applyAutoCorrection = () => {
    const found = window.findAutoCorrectionPoints(parsed, [0.4, 0.8]);
    if (!found) {
      alert(T.autoCorrectFail || 'No se pudo corregir automáticamente: la probeta no alcanzó 1 kN, o no se hallaron los puntos de referencia (0.4 / 0.8 kN).');
      return;
    }
    onUpdate({ ...specimen, correction: found, trimIdx: null });
  };
    ...
    <div className="viewer-individual">
      <div className="viewer-toolbar">
        <div className="vt-group">
          ...
          {allowAutoCorrection && (
            <button className="vt-btn" onClick={applyAutoCorrection}
                    title={T.autoCorrectHint || 'Corrección automática: recta entre el punto más cercano a 0.4 kN y el más cercano a 0.8 kN (primer cruce). Solo si la probeta alcanzó 1 kN.'}>
              ⚡ {T.autoCorrect || 'Corrección automática'}
            </button>
          )}
...
function ComparatorView({ state, T, lang }) {
  ...
  const [comboColors, setComboColors] = React.useState({}); // sig -> color editado por el usuario
  ...
  const toggleColorBy = (f) => setColorBy(arr => arr.includes(f) ? arr.filter(x => x !== f) : [...arr, f]);
  const sigOf = (mix) => {
    const d = designByRun[mix];
    if (!d) return null;
    return colorBy.map(f => `${f}${d[f]}`).join(' · ');
  };
  // Mapa combinación-de-factores → color, en orden de aparición entre las curvas seleccionadas.
  // Por defecto: 1 solo factor → azul(−)/gris(0)/rojo(+); varios factores → paleta. Editable por el usuario.
  const colorCombos = React.useMemo(() => {
    const map = new Map();
    if (!colorBy.length) return map;
    let idx = 0;
    selected.forEach(s => {
      const d = designByRun[s.mix];
      if (!d) return;
      const sig = sigOf(s.mix);
      if (sig == null || map.has(sig)) return;
      const def = colorBy.length === 1
        ? (LEVEL_COLOR[d[colorBy[0]]] || PALETTE[idx % PALETTE.length])
        : PALETTE[idx % PALETTE.length];
      map.set(sig, { sig, def, color: comboColors[sig] || def, idx });
      idx++;
    });
    return map;
  ...
  // Codificación por factor del diseño: color (combinación de factores) y/o estilo de línea.
  if (colorBy.length || lineBy !== 'none') {
    const mixByKey = {}; selected.forEach(s => { mixByKey[s.key] = s.mix; });
    series = series.map(sr => {
      const mix = mixByKey[sr.key];
      const d = designByRun[mix];
      if (!d) return sr;
      const next = { ...sr };
      if (colorBy.length) {
        const info = colorCombos.get(sigOf(mix));
        if (info) next.color = info.color;
      }
  ...
  if (showGhost && viewState === 'processed' && (!hasShrink || hasMech)) {
    selected.filter(s => visibleMap[s.key] !== false && s.kind === 'mech').forEach(s => {
      ...
      if (colorBy.length) { const info = colorCombos.get(sigOf(s.mix)); if (info) gColor = info.color; }
      else if (gd) gColor = colorMap[s.key] || '#888';
      ...
      <div className="comp-header">
        ...
        <div className="comp-actions">
          ...
          <button className="vt-btn" onClick={() => setSelected([])} disabled={selected.length === 0}
                  title={T.clearAllHint || 'Quita todas las curvas del comparador (no borra sus modificaciones)'}>
      ...
      {selected.length > 0 && (
        <div className="comp-controls comp-encode">
          <div className="vt-group">
            ...
            <span className="enc-sel">{T.colorByLabel || 'Color'}</span>
            <div className="enc-chips">
              {COMP_FACTORS.map(f => (
                <button key={f} className={'enc-chip' + (colorBy.includes(f) ? ' on' : '')}
                        onClick={() => toggleColorBy(f)}>{f}</button>
              ))}
            </div>
          ...
          {(colorBy.length > 0 || lineBy !== 'none') && (
            <div className="enc-legend">
              {colorBy.length > 0 && [...colorCombos.values()].map(info => (
                <span key={info.sig} className="enc-item">
                  <input type="color" className="enc-color" value={info.color}
                         title={T.editColor || 'Editar color'}
                         onChange={(e) => setComboColor(info.sig, e.target.value)} />
                  {info.sig}
                </span>
              ))}
              {lineBy !== 'none' && ['-', '0', '+'].map(lv => (
                <span key={'l' + lv} className="enc-item">
                  <svg width="34" height="12"><line x1="1" y1="6" x2="33" y2="6"
                    stroke="var(--text-2)" strokeWidth="2.4"
                    strokeDasharray={LEVEL_DASH[lv] !== 'none' ? LEVEL_DASH[lv] : undefined} /></svg>
                  {lineBy} {LEVEL_NAME[lv]}
                </span>
              ))}
            </div>
          )}
