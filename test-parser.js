...
window.parseTestFile = function(text) {
  ...
  // Recortar los puntos iniciales con fuerza = 0: la curva empieza en el primer
  // punto con carga distinta de 0. Se conserva TODO lo posterior (incluida la
  // rama de post-falla después de la carga máxima).
  let startIdx = points.findIndex(p => Math.abs(p.load) > 0);
  if (startIdx > 0) {
  ...
  // Downsample para mantener los documentos de Firestore bajo el límite de 1 MB.
  // Conservamos el punto de P máx para no perder el pico.
  const nTotal = points.length;
  let outPoints = points;
  let outIdxPmax = idxPmax;
  const CAP = 600;
  if (nTotal > CAP) {
    const step = Math.ceil(nTotal / CAP);
    const reduced = [];
    let newIdxPmax = 0;
    for (let i = 0; i < nTotal; i += step) {
      if (idxPmax >= i && idxPmax < i + step) newIdxPmax = reduced.length;
      reduced.push(points[i]);
    }
    // asegurar que el punto de pico esté presente
    if (points[idxPmax] && reduced[newIdxPmax] !== points[idxPmax]) {
      reduced[newIdxPmax] = points[idxPmax];
    }
    outPoints = reduced;
    outIdxPmax = newIdxPmax;
  }
  return {
    meta,
    points: outPoints,
...
window.migrateMixSpecs = function(mix, mixData, user) {
  ...
  for (const test of supportedTests) {
    ...
    // Si el usuario no declara probetas para este ensayo, eliminar todo (ej. retracción en vértices)
    out[test] = specsDef.length === 0 ? [] : merged;
  }
