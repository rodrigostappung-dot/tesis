...
window.migrateMixSpecs = function(mix, mixData) {
  ...
  for (const test of window.TESTS) {
    ...
    // Append leftover unknown specs at end (no perdemos datos del usuario)
...
window.makeEmptyResults = function() {
  ...
  for (let mix = 1; mix <= 45; mix++) {
    data[mix] = {
      retraction: window.getSpecsFor(mix, 'retraction').map(s => emptyShrinkSpec(s.id)),
      flexion: window.getSpecsFor(mix, 'flexion').map(s => emptyMechSpec(s.id, s.age)),
      compression: window.getSpecsFor(mix, 'compression').map(s => emptyMechSpec(s.id, s.age)),
    };
  }
  return data;
};
