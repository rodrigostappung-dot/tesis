  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.results && parsed.mixMeta) {
        // Migrate all mixes to match current spec definitions (M45 has special layout)
        for (let i = 1; i <= 45; i++) {
          if (parsed.results[i]) parsed.results[i] = window.migrateMixSpecs(i, parsed.results[i]);
        }
...
function ShrinkageSection({ specimens, onUpdate, onOpenViewer, T }) {
      ...
      window.FB.onReady(async () => {
        ...
        try {
          ...
          if (remote && Object.keys(remote.results).length > 0) {
            // Migrate remote data to current spec layout (M45 etc.)
            for (let i = 1; i <= 45; i++) {
              if (remote.results[i]) remote.results[i] = window.migrateMixSpecs(i, remote.results[i]);
            }
            setState(remote);
