    ...
    for (const m in compact.results) {
      for (const t of ['flexion', 'compression']) {
        ...
        for (const r of specs) {
          if (r.parsed && Array.isArray(r.parsed.points)) {
            let pts = r.parsed.points;
            if (pts.length > 500) {
              const step = Math.ceil(pts.length / 500);
              pts = pts.filter((_, i) => i % step === 0);
            }
            // Conservar solo los campos usados (reduce ~3x el tamaño)
            r.parsed.points = pts.map(p => ({ t: p.t, load: p.load, disp: p.disp }));
          }
        }
      }
    }
...
function MixTile({ mix, design, mixData, expanded, onClick, T, user }) {
  ...
  const isCenter = user?.centerMix === mix;
  const cells = [];
  // Shrink ages (solo si la config del usuario define retracción para esta mezcla;
  // así un vértice nunca muestra cuadrados aunque queden datos viejos en la nube)
  const hasRetrSpecs = window.getSpecsForUser(user, mix, 'retraction').length > 0;
  const shrinkAges = hasRetrSpecs ? window.getShrinkAgesForMix(user, mixData) : [];
...
function App() {
  ...
  // Migración: reescribe TODAS las mezclas en la nube en el formato compacto.
  // No requiere volver a subir archivos: usa lo ya cargado desde la nube.
  const [migrating, setMigrating] = useState(null); // null | 'N/total'
  // Corrección automática masiva: aplica la corrección automática de flexión
  // (0.4 kN / 0.8 kN, solo si llegó a 1 kN) a TODAS las probetas de flexión de
  // todas las mezclas. No toca las que ya tienen una corrección (manual o previa).
  const [bulkCorrecting, setBulkCorrecting] = useState(false);
  const [showExportModal, setShowExportModal] = useState(false);
  const [exportFields, setExportFields] = useState(() => ({ ...window.EXPORT_FIELDS_DEFAULT }));
  const bulkAutoCorrectFlexion = () => {
    if (!user || bulkCorrecting) return;
    if (!confirm(T.bulkCorrectConfirm || 'Aplicar corrección automática a TODAS las probetas de flexión que aún no tengan corrección? (Requiere haber llegado a 1 kN)')) return;
    setBulkCorrecting(true);
    let corrected = 0, skippedHasCorr = 0, skippedNoReach = 0, total = 0;
    const nextResults = { ...state.results };
    const touchedMixes = [];
    for (let mix = 1; mix <= user.mixCount; mix++) {
      const md = nextResults[mix];
      if (!md || !Array.isArray(md.flexion) || !md.flexion.length) continue;
      let changed = false;
      const newFlexion = md.flexion.map(spec => {
        if (!spec.parsed || !spec.parsed.points || !spec.parsed.points.length) return spec;
        total++;
        if (spec.correction) { skippedHasCorr++; return spec; }
        const found = window.findAutoCorrectionPoints(spec.parsed, [0.4, 0.8]);
        if (!found) { skippedNoReach++; return spec; }
        corrected++; changed = true;
        return { ...spec, correction: found, trimIdx: null };
      });
      if (changed) { nextResults[mix] = { ...md, flexion: newFlexion }; touchedMixes.push(mix); }
    }
    setState(s => ({ ...s, results: nextResults }));
    setDirty(prev => { const n = new Set(prev); touchedMixes.forEach(m => n.add(m)); return n; });
    setBulkCorrecting(false);
    alert(`Corrección automática de flexión completa.\nCorregidas: ${corrected}\nYa tenían corrección: ${skippedHasCorr}\nNo llegaron a 1 kN: ${skippedNoReach}\nTotal revisadas: ${total}`);
  };

  const migrateAllToCloud = async () => {
    if (!fbReady || !user) return;
    if (!confirm('Reescribir las ' + user.mixCount + ' mezclas en la nube con el nuevo formato compacto? No borra datos, solo los actualiza.')) return;
    const all = Array.from({length: user.mixCount}, (_, i) => i + 1);
    let doneN = 0;
    try {
      for (let i = 0; i < all.length; i += 5) {
        const batch = all.slice(i, i + 5);
        await window.FB.saveMany(batch, state, user);
        doneN += batch.length;
        setMigrating(doneN + '/' + all.length);
      }
      setDirty(new Set());
      setFbLastSync(new Date());
      setMigrating(null);
      alert('Listo: ' + all.length + ' mezclas actualizadas al formato compacto.');
    } catch (e) {
      console.error(e);
      setMigrating(null);
      alert('Error migrando: ' + (e.message || e));
    }
  };
  ...
  return (
    <div className="app">
      ...
      {showExportModal && (
        <div className="cal-modal-backdrop" onClick={() => setShowExportModal(false)}>
          <div className="cal-modal" style={{maxWidth: 480, width: '92%'}} onClick={(e) => e.stopPropagation()}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10}}>
              <h2 style={{margin: 0, fontSize: 16, fontWeight: 600}}>{T.exportModalTitle || 'Elegir variables a exportar'}</h2>
              <button className="cal-close" onClick={() => setShowExportModal(false)}>✕</button>
            </div>
            <div style={{fontSize: 12, color: 'var(--text-3)', marginBottom: 12}}>
              {T.exportModalHint || 'Se exportan 2 hojas (Compresión y Flexión), una fila por mezcla (N45 en 3 réplicas), con las variables marcadas para cada probeta/edad.'}
            </div>
            <div style={{display: 'flex', gap: 8, marginBottom: 12}}>
              <button className="vt-btn" onClick={() => setExportFields({ ...window.EXPORT_FIELDS_DEFAULT })}>
                {T.selectAll || 'Marcar todas'}
              </button>
              <button className="vt-btn" onClick={() => setExportFields({})}>
                {T.selectNone || 'Desmarcar todas'}
              </button>
            </div>
            <div style={{display: 'flex', flexDirection: 'column', gap: 8, maxHeight: '50vh', overflowY: 'auto', paddingRight: 4}}>
              {window.EXPORT_FIELDS.map(f => (
                <label key={f.id} className="vt-check" style={{fontSize: 13}}>
                  <input type="checkbox" checked={!!exportFields[f.id]}
                         onChange={(e) => setExportFields(s => ({ ...s, [f.id]: e.target.checked }))} />
                  {f.label}
                  {f.compOnly && <span style={{color: 'var(--text-3)', fontSize: 11}}> (solo compresión)</span>}
                </label>
              ))}
            </div>
            <div style={{display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 14}}>
              <button className="vt-btn" onClick={() => setShowExportModal(false)}>{T.cancel || 'Cancelar'}</button>
              <button className="vt-btn primary" onClick={() => {
                try { window.exportMixesExcel(state, user, T, exportFields); setShowExportModal(false); }
                catch (e) { console.error(e); alert('Error al exportar: ' + (e.message || e)); }
              }}>⬇ {T.exportExcel || 'Exportar'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'viewer' && (
        <div className="viewer-tab">
          ...
          <div style={{marginTop: 24, borderTop: '1px solid var(--border)', paddingTop: 20, display: 'flex', alignItems: 'center', gap: 12}}>
            <button className="vt-btn primary" style={{fontSize: 14, padding: '10px 18px'}}
              onClick={() => setShowComparator(true)}>
              📊 {T.openComparator || 'Abrir comparador de curvas'}
            </button>
            <span style={{fontSize: 12, color: 'var(--text-3)'}}>
              {T.comparatorHint || 'Superpone y compara varias curvas en una ventana.'}
            </span>
          </div>

          {showComparator && (
            <div className="cal-modal-backdrop" onClick={() => setShowComparator(false)}>
              <div className="cal-modal" style={{maxWidth: 1180, width: '97%'}} onClick={(e) => e.stopPropagation()}>
                <div style={{display: 'flex', justifyContent: 'flex-end', marginBottom: 4}}>
                  <button className="cal-close" onClick={() => setShowComparator(false)}>✕</button>
                </div>
